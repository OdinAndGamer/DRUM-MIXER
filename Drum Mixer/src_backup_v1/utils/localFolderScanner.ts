...
export async function scanLocalDirectory(
  ...
  async function walk(
    ...
    if (audioFiles.length > 0) {
      ...
      const wavFiles = files.filter(f => /\.(wav|wave|mp3|m4a|ogg|aac|wma|mp4|webm)$/i.test(f.name));
      ...
      if (flacFile || wavFiles.length > 0) {
        ...
        // Try to split on common delimiters: "–" (en-dash), "-" (hyphen), " - "
        const splitters = [' – ', ' - ', ' -', '- '];
        let splitSuccess = false;
        for (const splitter of splitters) {
          if (folderName.includes(splitter)) {
            const parts = folderName.split(splitter);
            title = parts[0].trim();
...
export async function scanLocalFilesList(
  ...
  for (const folderKey of folderKeys) {
    ...
    if (audioFiles.length > 0) {
      // Find non-flac files
      const wavFiles = audioFiles.filter(f => !/\.flac$/i.test(f.name));
