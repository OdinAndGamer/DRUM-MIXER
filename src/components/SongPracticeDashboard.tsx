...
export const SongPracticeDashboard: React.FC<SongPracticeDashboardProps> = ({
  ...
  const [showConfirmSongReset, setShowConfirmSongReset] = useState<boolean>(false);

  useEffect(() => {
    setShowConfirmSongReset(false);
    if (!currentSong) return;
  ...
  const handleResetLocalDashboardState = () => {
    ...
    setPracticeHistory([]);
    setLocalNote('');
    onSaveNote(''); // Clear note in App state & localStorage for current song
    if (currentSong) {
    ...
    <div className="flex flex-col gap-5" id="practice-dashboard-grid">
      ...
      <div className="bg-brand-card border border-brand-border rounded-xl p-4 flex flex-col justify-start">
        ...
        <div className="space-y-1.5 border-t border-brand-border/40 pt-2">
          ...
          {showConfirmSongReset ? (
            <div className="bg-red-950/30 border border-red-500/40 rounded-xl p-2.5 text-center space-y-2 mt-2">
              <span className="text-[10px] font-bold text-red-400 block leading-tight">
                Sicher? Notizen, Streaks, Übungs-Level und Statistiken für diesen Song zurücksetzen?
              </span>
              <div className="flex items-center justify-center gap-2">
                <button
                  ...
                  className="px-2.5 py-1 bg-red-600 hover:bg-red-500 text-white font-bold rounded text-[10px] transition cursor-pointer shadow-md shadow-red-950/50"
                  id="confirm-song-reset-btn"
                >
                  Ja, zurücksetzen
                </button>
                <button
                  type="button"
                  onClick={() => setShowConfirmSongReset(false)}
                  className="px-2.5 py-1 bg-black/60 hover:bg-black/80 text-slate-400 font-bold rounded text-[10px] border border-brand-border transition cursor-pointer"
                  id="cancel-song-reset-btn"
                >
                  Abbrechen
                </button>
              </div>
            ...
            (stats.totalDuration > 0 || stats.playCount > 0 || practiceHistory.length > 0 || practiceLevel > 0 || confidenceLevel > 0 || localNote.trim().length > 0) && (
              <div className="flex justify-end pt-1">
                <button
                  type="button"
                  onClick={() => setShowConfirmSongReset(true)}
                  ...
                  title="Notizen, Streaks & Statistik für diesen Song zurücksetzen"
                  id="trigger-song-reset-btn"
                >
                  <RotateCcw className="w-2.5 h-2.5" />
                  Notizen, Streaks & Stats zurücksetzen
                </button>
              </div>
            )
          )}
