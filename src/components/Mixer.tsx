...
export const Mixer: React.FC<MixerProps> = ({
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      // 1. Get real peaks from Web Audio API analysers!
      const realPeaks = audioEngine.getPeaks();

      // 2. Check if we have active real signals.
      const hasRealSignals = Object.values(realPeaks).some(v => v > 0.001);

      if (hasRealSignals) {
        // Use pristine physical peaks from the Web Audio analyser!
        setPeaks(realPeaks);
        prevPeaks = realPeaks;
      } else {
        // 3. Fallback simulation (e.g. during loading or early starts)
        // ONLY bounce channels that are ACTUALLY loaded/active in the audio engine and have volume > 0!
        const getTargetPeak = (track: TrackType) => {
          const volume = volumes[track];
          if (volume === 0 || !audioEngine.isTrackActive(track)) return 0;
