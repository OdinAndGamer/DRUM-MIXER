...
export default function App() {
  ...
  const loadSongIntoEngine = async (song: Song) => {
    ...
    try {
      ...
      const loadedDuration = audioEngine.getDuration();
      setSongDuration(loadedDuration);
      if (loadedDuration > 0) {
        song.duration = loadedDuration;
        setSongs((prev) => prev.map((s) => (s.id === song.id ? { ...s, duration: loadedDuration } : s)));
      }
    } catch (error) {
