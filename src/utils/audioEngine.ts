...
class DrumPracticeAudioEngine {
  ...
  private analysers: Partial<Record<TrackType | 'Master', AnalyserNode>> = {};
  ...
  private setupMasterGain() {
    ...
    masterAnalyser.connect(this.ctx.destination);
    this.analysers['Master'] = masterAnalyser;

    // Create channel gain nodes with analysers
  ...
  public getPeaks(): Record<TrackType | 'Master', number> {
    const peaks = { Drum: 0, Gesang: 0, Instrumente: 0, Klick: 0, Master: 0 };
    if (!this.ctx || !this.isPlaying || this.isPaused) return peaks;

    const channels: (TrackType | 'Master')[] = ['Drum', 'Gesang', 'Instrumente', 'Klick', 'Master'];
    const bufferLength = 32; // fftSize 64 / 2
    const dataArray = new Uint8Array(bufferLength);

    channels.forEach((chan) => {
      const analyser = this.analysers[chan];
      if (analyser) {
        // If a specific track is not Master, check if we actually have an active source or audio loaded
        if (chan !== 'Master' && !this.audios[chan as TrackType]) {
          peaks[chan] = 0;
          return;
        }
        ...
        // Compute peak value from time domain data (range 0 to 255, center is 128)
        let maxVal = 0;
        ...
        // Normalize value (max deviation is 128)
        const peak = maxVal / 128;
        
        // Apply slight boost factor to make the meter bounce look lively and prominent
        peaks[chan] = Math.min(1.0, peak * 2.2);
      }
    });
  ...
  public isTrackActive(track: TrackType): boolean {
    return !!this.audios[track];
  }
